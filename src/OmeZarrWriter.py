...
class OmeZarrWriter(OmeWriter):
    def write(self, filename, source, zarr_version=2, ome_version='0.4'):
        ...
        nchannels = max(len(self.source.metadata['channels']), 1)
        dim_order = 'yx'
        if nchannels > 1:
        ...
        for well_id, zone_id in source.metadata['wells'].items():
            ...
            for field_index, field in enumerate(field_paths):
                ...
                data = self.source.get_field_image(well_id, field_index)
                write_image(image=data, group=image_group, axes=axes, fmt=ome_format)

        #channels = source.metadata['well_info']['channels']
